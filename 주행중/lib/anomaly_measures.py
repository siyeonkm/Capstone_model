...
이상탐지를 위한 측정
Input
1) bounding box : 현재 관측치, 예측치
2) bounding box's ego motion : 현재 관측치, 예측치
'''
import numpy as np
...
def iou_metrics(all_pred_boxes_t, all_observed_boxes, multi_box='average', iou_type='average'):
    '''
    Params:
...
def mask_iou_metrics(all_pred_boxes_t, all_observed_boxes, W, H, multi_box='latest'):
    '''
    ...
    return 1-mask_iou, pred_mask, observed_mask
